...
public class Car implements Runnable{
	...
	if (cartype==REDCAR) {
            if (inFront.getX() - xpos > 100) {
                xpos += 4;
                if (xpos >= bridgeXLeft & xposOld < bridgeXLeft) controller.redEnters();
		...
		else if (xpos >= bridgeXRight &&  xposOld < bridgeXRight) controller.redExits();
            }
	} else {
            if (xpos-inFront.getX() > 100) {
                xpos -= 4;
                if (xpos <= bridgeXRight && xposOld > bridgeXRight) controller.blueEnters();
		...
		else if (xpos <= bridgeXLeft && xposOld > bridgeXLeft) controller.blueExits();
            }
	}
    }
